        ...
        <ModalBase :visible="showModal" :mode="editing ? 'edit' : 'create'" entityName="personal"
            ...
            <template #content>
                <form @submit.prevent="handleSubmit" class="needs-validation" novalidate>
                    <!-- Pestañas PrimeVue CORREGIDAS -->
                    <div class="mb-4">
                        <!-- CAMBIO AQUÍ: Usar Tabs en lugar de TabView -->
                        <Tabs value="0" class="custom-tabs">
                            <TabList>
                                <Tab value="0">Información General</Tab>
                                <Tab value="1">Datos Laborales</Tab>
                                <Tab value="2">Emergencia</Tab>
                            </TabList>
                            <TabPanels>
                                <!-- Información General -->
                                <TabPanel value="0">
                                    <div class="space-y-4 mt-3">
                                        ...
                                        <div class="grid grid-cols-4 gap-4">
                                            <!-- Columna 1: Nombre -->
                                            <div class="h-20">
                                                <FloatInput id="nombre" label="Nombres" v-model="form.nombre"
                                                    icon="pi pi-user" :errors="errors" :invalid="!!errors.nombre"
                                                    size="small" required @input="convertToUppercase('nombre')" />
                                            </div>

                                            <!-- Columna 2: Apellido -->
                                            <div class="h-20">
                                                <FloatInput id="apellido" label="Apellidos" v-model="form.apellido"
                                                    icon="pi pi-user" :invalid="!!errors.apellido" :errors="errors"
                                                    size="small" required @input="convertToUppercase('apellido')" />
                                            </div>

                                            <!-- Columna 3: DNI -->
                                            <div class="h-20">
                                                <FloatInput id="dni" label="DNI" v-model="form.dni" icon="pi pi-id-card"
                                                    :errors="errors" :invalid="!!errors.dni" size="small" type="tel"
                                                    required maxlength="8" @input="onlyNumbersDNI" />
                                            </div>

                                            <!-- Columna 4: RUC -->
                                            <div class="h-20">
                                                <FloatInput id="ruc" label="RUC" v-model="form.ruc" icon="pi pi-id-card"
                                                    :errors="errors" :invalid="!!errors.ruc" size="small" maxlength="11"
                                        ...
                                        <div class="grid grid-cols-4 gap-4">
                                            ...
                                            <!-- Columna 2: Email Personal -->
                                            <div class="h-20">
                                                <FloatInput id="email_per" label="Email Personal"
                                                    v-model="form.email_per" type="email" icon="pi pi-envelope"
                                                    :invalid="!!errors.email_per" :errors="errors" size="small" />
                                            </div>
                                        ...
                                        <div class="grid grid-cols-4 gap-4">
                                            ...
                                            <div class="h-20">
                                                <FloatLabel>
                                                    <DatePicker v-model="form.fecha_nac" id="fecha_nac" :showIcon="true"
                                                        class="w-full" size="small"
                                                        :class="{ 'p-invalid': !!errors.fecha_nac }" inputId="fecha_nac"
                                                        :showClear="true" placeholder="Seleccionar fecha" />
                                                    <label for="fecha_nac" class="text-sm font-medium text-gray-700">
                                                        <i class="pi pi-calendar mr-2"></i>
                                                        Fecha de Nacimiento
                                                    </label>
                                                </FloatLabel>
                                                <div v-if="errors.fecha_nac" class="p-error text-sm mt-1">
                                                    {{ errors.fecha_nac[0] }}
                                                </div>
                                            </div>
                                            ...
                                            <div class="h-20">
                                                <Dropdown id="sexo" v-model="form.sexo" :options="sexoOptions"
                                                    ...
                                                    :class="{ 'p-invalid': !!errors.sexo }" class="w-full" size="small"
                                                    :showClear="true">
                                                    <template #option="slotProps">
                                                        <div class="flex items-center">
                                                            <i :class="slotProps.option.value === 'M' ? 'pi pi-mars text-blue-500' : 'pi pi-venus text-pink-500'"
                                                                class="text-base mr-2"></i>
                                                            <span>{{ slotProps.option.label }}</span>
                                                        </div>
                                                    </template>
                                                </Dropdown>
                                                <small v-if="errors.sexo" class="p-error text-sm mt-1 block">
                                                    {{ errors.sexo[0] }}
                                                </small>
                                            </div>

                                            <!-- Columna 3: Es Padre / Madre -->
                                            <div class="h-20">
                                                <Dropdown id="padre_madre" v-model="form.padre_madre"
                                                    :options="opcionesSiNo" optionLabel="label" optionValue="value"
                                                    placeholder="Seleccionar"
                                                    :class="{ 'p-invalid': !!errors.padre_madre }" class="w-full"
                                                    size="small" :showClear="true" @change="handlePadreMadreChange">
                                                </Dropdown>

                                                <small v-if="errors.padre_madre" class="p-error text-sm mt-1 block">
                                                    {{ errors.padre_madre[0] }}
                                                </small>
                                            </div>

                                            <!-- Columna 4: Número de hijos -->
                                            <div class="h-20">
                                                <FloatLabel>
                                                    <Dropdown id="n_hijos" v-model="form.n_hijos"
                                                        :options="hijosOptions" optionLabel="label" optionValue="value"
                                                        placeholder="N° hijos"
                                                        :class="{ 'p-invalid': !!errors.n_hijos, 'opacity-50': form.padre_madre !== 'Si' }"
                                                        class="w-full" size="small"
                                                        :disabled="form.padre_madre !== 'Si'" :showClear="true" />
                                                </FloatLabel>
                                                <small v-if="errors.n_hijos" class="p-error text-sm mt-1 block">
                                                    {{ errors.n_hijos[0] }}
                                                </small>
                                            </div>
                                        </div>
                                        <!-- Fila 4: Dirección y Distrito - CORREGIDO -->
                                        <div class="grid grid-cols-2 gap-4">
                                            <!-- Dirección -->
                                            <div class="h-20">
                                                <FloatInput id="direccion" label="Dirección" v-model="form.direccion"
                                                    icon="pi pi-map-marker" :invalid="!!errors.direccion"
                                                    :errors="errors" size="small" class="w-full" />
                                            </div>

                                            <!-- Distrito -->
                                            <div class="h-20">
                                                <FloatLabel>
                                                    <Dropdown id="distrito" v-model="form.distrito"
                                                        :options="distritosLima" optionLabel="label" optionValue="value"
                                                        placeholder="Seleccionar distrito"
                                                        :class="{ 'p-invalid': !!errors.distrito }" class="w-full"
                                                        size="small" :filter="true"
                                                        filterPlaceholder="Buscar distrito..." :showClear="true" />
                                                </FloatLabel>
                                                <small v-if="errors.distrito" class="p-error text-sm mt-1 block">
                                                    {{ errors.distrito[0] }}
                                                </small>
                                            </div>
                                        </div>
                                ...
                                <TabPanel value="1">
                                    <div class="mt-3">
                                        ...
                                        <div class="grid grid-cols-3 gap-4">
                                            ...
                                            <div class="h-24">
                                                <FloatLabel>
                                                    <Dropdown id="dependencia" v-model="form.dependencia"
                                                        :options="dependenciasOptions" optionLabel="nombre"
                                                        optionValue="id" placeholder="Seleccionar dependencia"
                                                        :class="{ 'p-invalid': !!errors.dependencia }" class="w-full"
                                                        :showClear="true" size="small" />
                                                </FloatLabel>
                                                <small v-if="errors.dependencia" class="p-error text-sm mt-1 block">
                                            ...
                                            <div class="h-24">
                                                <FloatLabel>
                                                    <Dropdown id="area" v-model="form.area" :options="areasOptions"
                                                        ...
                                                        :class="{ 'p-invalid': !!errors.area }" class="w-full"
                                                        :showClear="true" size="small" />

                                                </FloatLabel>
                                                <small v-if="errors.area" class="p-error text-sm mt-1 block">
                                                    {{ errors.area[0] }}
                                                </small>
                                                <small v-if="personalStore.loading"
                                                    class="text-blue-500 text-sm mt-1 block">
                                                    <i class="pi pi-spinner pi-spin mr-1"></i> Cargando áreas...
                                                </small>
                                            </div>
                                            ...
                                            <div class="h-24">
                                                <FloatLabel>
                                                    <Dropdown id="condicion" v-model="form.condicion"
                                                        :options="condicionOptions" optionLabel="nombre"
                                                        optionValue="id" placeholder="Seleccionar condición"
                                                        :class="{ 'p-invalid': !!errors.condicion }" class="w-full"
                                                        size="small" :showClear="true" />

                                                </FloatLabel>
                                                <small v-if="errors.condicion" class="p-error text-sm mt-1 block">
                                                    {{ errors.condicion[0] }}
                                                </small>
                                            </div>
                                        </div>
                                        ...
                                        <div class="grid grid-cols-3 gap-4">
                                            ...
                                            <div class="h-24">
                                                <FloatLabel>
                                                    <DatePicker v-model="form.fecha_inicio" id="fecha_inicio"
                                                        ...
                                                        :class="{ 'p-invalid': !!errors.fecha_inicio }"
                                                        inputId="fecha_inicio" :showClear="true"
                                                        placeholder="Seleccionar fecha" />
                                                </FloatLabel>
                                                <div v-if="errors.fecha_inicio" class="p-error text-sm mt-1">
                                            ...
                                            <div class="h-24">
                                                <FloatLabel>
                                                    <DatePicker v-model="form.fecha_fin" id="fecha_fin"
                                                        dateFormat="dd/mm/yy" :showIcon="true" class="w-full"
                                                        :class="{ 'p-invalid': !!errors.fecha_fin }" inputId="fecha_fin"
                                                        :showClear="true" placeholder="Seleccionar fecha" />
                                                </FloatLabel>
                                                <div v-if="errors.fecha_fin" class="p-error text-sm mt-1">
                                                    {{ errors.fecha_fin[0] }}
                                                </div>
                                            </div>
                                            ...
                                            <div class="h-24">
                                                <FloatLabel>
                                                    <Dropdown id="profesion" v-model="form.profesion"
                                                        :options="profesionOptions" optionLabel="nombre"
                                                        optionValue="id" placeholder="Seleccionar profesión"
                                                        :class="{ 'p-invalid': !!errors.profesion }" class="w-full"
                                                        :filter="true" filterPlaceholder="Buscar profesion..."
                                                        :showClear="true" size="small" />
                                                </FloatLabel>
                                                <small v-if="errors.profesion" class="p-error text-sm mt-1 block">
                                                    {{ errors.profesion[0] }}
                                                </small>
                                            </div>
                                        </div>
                                            ...
                                            <div class="h-24">
                                                <FloatLabel>
                                                    <Dropdown id="estado" v-model="form.estado" :options="estadoOptions"
                                                        ...
                                                        :class="{ 'p-invalid': !!errors.estado }" class="w-full"
                                                        :showClear="true" size="small" />
                                                </FloatLabel>
                                                <small v-if="errors.estado" class="p-error text-sm mt-1 block">
                                                    {{ errors.estado[0] }}
                                                </small>
                                            </div>
                                            ...
                                            <div class="h-24">
                                                <FloatLabel>
                                                    <Dropdown id="cargo" v-model="form.cargo" :options="cargoOptions"
                                                        ...
                                                        :class="{ 'p-invalid': !!errors.cargo }" class="w-full"
                                                        :filter="true" filterPlaceholder="Buscar cargo..."
                                                        :showClear="true" size="small" />
                                                </FloatLabel>
                                                <small v-if="errors.cargo" class="p-error text-sm mt-1 block">
                                                    {{ errors.cargo[0] }}
                                                </small>
                                            </div>
                                            ...
                                            <div class="h-24">
                                                <FloatLabel>
                                                    <Dropdown id="anexo" v-model="form.anexo" :options="anexoOptions"
                                                        ...
                                                        :class="{ 'p-invalid': !!errors.anexo }" class="w-full"
                                                        :showClear="true" size="small" :filter="true"
                                                        filterPlaceholder="Buscar anexo..." />
                                                </FloatLabel>
                                                <small v-if="errors.anexo" class="p-error text-sm mt-1 block">
                                                    {{ errors.anexo[0] }}
                                                </small>
                                            </div>
                                        </div>
                                        ...
                                        <!-- Fila 4: 3 columnas -->
                                        <div class="grid grid-cols-3 gap-4">
                                            <!-- Columna 1: Grupo Ocupacional -->
                                            <div class="h-24">
                                                <FloatLabel>
                                                    <Dropdown id="grupo_ocupacional" v-model="form.grupo_ocupacional"
                                                        :options="grupoOcupacionalOptions" optionLabel="nombre"
                                                        optionValue="id" placeholder="Seleccionar grupo ocupacional"
                                                        :class="{ 'p-invalid': !!errors.grupo_ocupacional }"
                                                        class="w-full" :showClear="true" size="small" />
                                                </FloatLabel>
                                                <small v-if="errors.grupo_ocupacional"
                                                    class="p-error text-sm mt-1 block">
                                            ...
                                            <div class="h-24">
                                                <FloatLabel>
                                                    <Dropdown id="generica" v-model="form.generica"
                                                        :options="genericaOptions" optionLabel="nombre" optionValue="id"
                                                        placeholder="Seleccionar genérica"
                                                        :class="{ 'p-invalid': !!errors.generica }" class="w-full"
                                                        :showClear="true" size="small" />
                                                </FloatLabel>
                                                <small v-if="errors.generica" class="p-error text-sm mt-1 block">
                                                    {{ errors.generica[0] }}
                                                </small>
                                            </div>
                                        ...
                                        <div class="grid grid-cols-3 gap-4">
                                            ...
                                            <div class="h-24">
                                                <FloatLabel>
                                                    <Dropdown id="nivel" v-model="form.nivel" :options="nivelOptions"
                                                        ...
                                                        :class="{ 'p-invalid': !!errors.nivel }" class="w-full"
                                                        :showClear="true" size="small" />
                                                </FloatLabel>
                                                <small v-if="errors.nivel" class="p-error text-sm mt-1 block">
                                                    {{ errors.nivel[0] }}
                                                </small>
                                            </div>
                                            ...
                                            <div class="h-24">
                                                <FloatLabel>
                                                    <Dropdown id="regimen" v-model="form.regimen"
                                                        :options="regimenOptions" optionLabel="nombre" optionValue="id"
                                                        placeholder="Seleccionar régimen"
                                                        :class="{ 'p-invalid': !!errors.regimen }" class="w-full"
                                                        :showClear="true" size="small" />
                                                </FloatLabel>
                                                <small v-if="errors.regimen" class="p-error text-sm block">
                                                    {{ errors.regimen[0] }}
                                                </small>
                                            </div>

                                            <!-- Columna 3: Número de contrato -->
                                            <div class="h-24">
                                                <FloatInput id="n_contrato" label="Número de Contrato"
                                                    v-model="form.n_contrato" icon="pi pi-file-edit"
                                                    :invalid="!!errors.n_contrato" :errors="errors" size="small"
                                                    class="w-full" />
                                            </div>
                                        </div>
                                    </div>
