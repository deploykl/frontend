<template>
  <div
    class="bg-linear-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-blue-900/10 dark:to-indigo-900/20 relative overflow-hidden">
    ...
    <div class="relative z-10 container mx-auto px-1">
      <!-- Header elegante -->
      <div class="text-center mb-0">
        <div class="relative inline-flex flex-col items-center">
          ...
          <!-- Texto principal -->
          <h1
            class="text-4xl md:text-5xl font-bold bg-linear-to-r from-slate-800 via-blue-600 to-cyan-600 dark:from-slate-100 dark:via-blue-400 dark:to-cyan-400 bg-clip-text text-transparent mb-3 tracking-tight">
          ...
          <div class="relative">
            ...
            <div class="absolute inset-0 w-24 h-0.5 bg-linear-to-r from-cyan-500 to-blue-500 rounded-full animate-ping">
            </div>
          </div>

          <!-- Subtítulo -->
          <p class="mt-1 text-slate-600 dark:text-slate-400 text-lg font-light">
      ...
      <div class="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div class="space-y-6">
          ...
          <div
            class="bg-white/80 dark:bg-slate-800/80 backdrop-blur-lg rounded-2xl p-5 border border-slate-200/50 dark:border-slate-700/50 shadow-sm">
            <h3
              class="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-4 text-center uppercase tracking-wide">
              Selecciona el Mes</h3>
            <div class="grid grid-cols-4 gap-2">
              <button v-for="month in meses" :key="month.value" @click="cambiarMes(month.value)"
                class="aspect-square rounded-xl transition-all duration-300 border-2 group hover:scale-105 cursor-pointer"
                ...
                <div class="flex flex-col items-center justify-center h-full p-1">
                  <span class="font-semibold text-xs group-hover:scale-110 transition-transform">{{ month.name }}</span>
                  <span class="text-xs opacity-80 mt-1">{{ mainStore.cumpleanosPorMes[month.value]?.length || 0
                    }}</span>
                </div>
              </button>
            </div>
          ...
          <div
            class="bg-linear-to-br from-cyan-500/10 to-blue-500/10 dark:from-cyan-500/5 dark:to-blue-500/5 backdrop-blur-lg rounded-2xl p-5 border border-cyan-200/30 dark:border-cyan-500/20">
            <div class="text-center space-y-4">
              ...
              <h3
                class="text-lg font-semibold bg-linear-to-r from-cyan-600 to-blue-600 dark:from-cyan-400 dark:to-blue-400 bg-clip-text text-transparent">
                ¡Feliz Cumpleaños!
              </h3>
              ...
              <button @click="toggleCelebration"
                ...
                <div class="flex items-center justify-center space-x-2">
                  <i class="pi text-sm transition-transform group-hover:scale-110"
                    :class="showCelebration ? 'pi-star-fill' : 'pi-star'"></i>
        ...
        <div class="xl:col-span-2">
          <div
            class="bg-white/80 dark:bg-slate-800/80 backdrop-blur-lg rounded-2xl border border-slate-200/50 dark:border-slate-700/50 shadow-sm overflow-hidden">
            ...
            <div class="p-5">
              ...
              <div v-if="mainStore.loading" class="flex flex-col items-center justify-center py-12">
                <div class="relative">
                  ...
                  <div
                    class="w-8 h-8 border-3 border-cyan-500 border-t-transparent rounded-full animate-spin absolute top-0 left-0">
                  </div>
                </div>
              ...
              <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div v-for="persona in mainStore.cumpleanosDelMesActual" :key="persona.id"
                  class="group p-2 rounded-xl border border-slate-200/60 dark:border-slate-600/60 bg-white/50 dark:bg-slate-700/30 backdrop-blur-sm transition-all duration-300 hover:shadow-md hover:border-cyan-300/50 dark:hover:border-cyan-500/30 hover:-translate-y-0.5"
                  ...
                  <div class="flex items-center space-x-4">
                    ...
                    <div class="flex-1 min-w-0">
                      <h3
                        class="font-semibold text-slate-900 dark:text-slate-100 text-sm mb-1 group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors truncate">
                        {{ persona.full_name }}
                      </h3>
                    ...
                    <div v-if="persona.dias_para_cumple === 0 && showCelebration" class="shrink-0">
                      <span
                        class="px-2.5 py-1 bg-linear-to-r from-amber-400 to-orange-500 text-white text-xs font-semibold rounded-full shadow-sm animate-pulse">
                        HOY
                      </span>
                    </div>
                  </div>
            ...
            <!-- Firma elegante -->
            <div v-if="mainStore.cumpleanosDelMesActual.length > 0"
              class="px-5 py-4 border-t border-slate-200/40 dark:border-slate-600/40 bg-white/30 dark:bg-slate-800/30">
...
// Meses abreviados para el panel izquierdo
const meses = [
...
// Nombre completo del mes para el panel derecho
const nombreMesCompleto = computed(() => {
  const mesesCompletos = [
    'Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio',
    'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre'
  ]
  return mesesCompletos[mainStore.currentMonth - 1] || ''
})
