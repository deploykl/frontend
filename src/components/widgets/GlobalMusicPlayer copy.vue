<template>
    <!-- Botón flotante para abrir el panel de música -->
       <button v-if="!showMusicPanel" @click="toggleMusicPanel"
        ...
        <!-- ESTADO: REPRODUCIENDO - Solo ondas -->
        <div v-if="isPlaying && currentTrack" class="flex space-x-1">
            <div class="w-1 h-3 bg-white rounded-full animate-bounce-slow"></div>
            <div class="w-1 h-3 bg-white rounded-full animate-bounce-slow" style="animation-delay: 0.1s"></div>
            <div class="w-1 h-3 bg-white rounded-full animate-bounce-slow" style="animation-delay: 0.2s"></div>
        </div>
        
        <!-- ESTADO: PAUSADO CON CANCIÓN - Icono play + borde giratorio -->
        <div v-else-if="!isPlaying && currentTrack" class="relative">
            <i class="pi pi-play text-white text-xl animate-pulse"></i>
            <div class="absolute inset-0 border-2 border-cyan-300 border-t-transparent rounded-full animate-spin-slow opacity-50"></div>
        </div>
        
        <!-- ESTADO: SIN CANCIÓN - Solo icono play normal -->
        <i v-else class="pi pi-play text-white text-xl"></i>
        
        <!-- Indicador de canción actual (solo cuando está pausado) -->
...
/* Animación especial para el borde giratorio */
.animate-spin-border {
